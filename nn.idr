...
data Gate : Type -> Type -> Type where
  ...
  Parameter : Show a => a -> (s -> a) -> (s -> a -> s) -> Gate a s
...
data Evaluated : Type -> Type -> Type where
  ...
  EvaluatedParameter : Show a => a -> (s -> a) -> (s -> a -> s) -> Evaluated a s
...
Show (Evaluated a s) where
  ...
  show (EvaluatedParameter x _ _)  = show x
...
value (EvaluatedParameter x _ _) = x
...
aGate =  (Vect [Fixed 2, (Parameter 3 p1 s1) + (Fixed 1)]) . (Vect [Fixed 4, Fixed 5])
...
evaluate (Parameter x get set) = EvaluatedParameter x get set
...
y = EvaluatedVect [EvaluatedAdd (EvaluatedParameter 2 p1 s1) (EvaluatedFixed 0) 2, EvaluatedFixed 2] [2, 2]
...
dot = Vect [Fixed 1, Fixed 2, Fixed 3] . Vect [Fixed 2, Parameter 3 p1 s1, Fixed 4] 
